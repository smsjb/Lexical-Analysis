...
class tablefc {
	...
	//讀指令 
	 read( string & fname){
		...
		ifstream _file( fname.c_str()) ; // 把名為fname的檔案打開 
		if (_file.is_open()) { //檔案存在 
		
	  		while ( !_file.eof() ) { //不是最後一行 
					...
					for(int i =0; i<alltb.six.size() && type==2; i++){  //check int/real in table
						...
						for (int j=0; j <token.size(); j++){  
							if((int)alltb.six[i][j]>=97 && (int)alltb.six[i][j]<=122) //a~z 
       							token[j] = tolower(token[j]);  //大寫變小寫  
       						else
       							token[j] = toupper(token[j]); //小寫變大寫 
						}
	...
	void hash( string str, int tb){
		...
		int ip =sum(str)%100; //算位置 
		
		if(tb==5){
		
				while(alltb.five[ip]!="\0"){  //碰撞 
					ip++;
					if(ip>=100)
						ip=ip%100;
				}
		...
		for(int i =0; i<alltb.one.size() && !out; i++){ 
			...
			for (int j=0; j <key.size(); j++){  
				if((int)alltb.one[i][j]>=97 && (int)alltb.one[i][j]<=122)  //a~z 
       				key[j] = tolower(key[j]);  //大寫變小寫  
       			else
       				key[j] = toupper(key[j]); //小寫變大寫 
			}
